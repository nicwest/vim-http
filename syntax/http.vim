...
syn match httpURILine       '^\(OPTIONS\|GET\|HEAD\|POST\|PUT\|DELETE\|TRACE\|CONNECT\)\( .* HTTP/[0-9.]\+\)$'  contains=httpMethod,httpProto contained
...
syn region httpHeader       start='^\(OPTIONS\|GET\|HEAD\|POST\|PUT\|DELETE\|TRACE\|CONNECT\)\( .* HTTP/[0-9.]\+\)$' end='\n\s*\n' contains=httpURILine,httpHeaderLine
